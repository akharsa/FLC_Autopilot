...
#define MPU6050
#define i2c_write(a, b, c, d)	!qI2C_Write(a,d,b,c) //result value inverted
#define i2c_read(a, b, c, d)    !qI2C_Read(a,d,b,c) //result value inverted
#define labs        abs
#define fabs(x)     (((x)>0)?(x):-(x))
#define delay_ms(x)	for(_delay_counter=0;_delay_counter<x*100000;_delay_counter++);
#define get_ms(x)
#define reg_int_cb(x)
#define log_e
#define log_i
...
int mpu_init(struct int_param_s *int_param)
{
    ...
    if (rev) {
        ...
        rev = data[0] & 0x0F;
        if (!rev) {
            log_i("Product ID read as 0 indicates device is either "
