...
#define TRCHOOKS_H
...
#if defined INCLUDE_NEW_TIME_EVENTS && INCLUDE_NEW_TIME_EVENTS != 0
    #define traceTASK_INCREMENT_TICK( xTickCount ) \
	  if (uxSchedulerSuspended == ( unsigned portBASE_TYPE ) pdFALSE) { vTraceStoreKernelCallWithNumericParamOnly(DIV_NEW_TIME, xTickCount + 1); } \
      else if (uxSchedulerSuspended == ( unsigned portBASE_TYPE ) pdTRUE || uxMissedTicks == 0) { extern uint32_t uiTraceTickCount; uiTraceTickCount++; uiTracePortGetTimeStamp(0); }
#else
    #define traceTASK_INCREMENT_TICK( xTickCount ) \
    ...
    #define traceTASK_DELAY() \
      portENTER_CRITICAL(); \
      vTraceStoreKernelCallWithNumericParamOnly(TASK_DELAY, xTicksToDelay);\
    ...
    #define traceTASK_DELAY_UNTIL() \
      portENTER_CRITICAL(); \
      vTraceStoreKernelCallWithNumericParamOnly(TASK_DELAY_UNTIL, xTimeToWake); \
      vTraceSetTaskInstanceFinished((uint8_t)pxCurrentTCB->uxTaskNumber); \
      portEXIT_CRITICAL();
