 ...
 * RAM. Using IAR Embedded Workbench for ARM, you may place RecorderData in 
 * this RAM bank using the following declaration.
 * 
 *     #pragma location="AHB_RAM_MEMORY" // A direct address also works
 ...
 * address section from the linker file, or a direct address. For further 
 * information, refer to the IAR help on "#pragma location".
 *
...
objectHandleType xTraceGetObjectHandle(traceObjectClass objectclass)
{
    ...
    else
    {
		...
		/* Clear any previous flags for this object */
		switch (objectclass)
		{
		case TRACE_CLASS_QUEUE:
			CLEAR_QUEUE_FLAG_ISEXCLUDED(handle);
			break;
		case TRACE_CLASS_SEMAPHORE:
			CLEAR_SEMAPHORE_FLAG_ISEXCLUDED(handle);
			break;
		case TRACE_CLASS_MUTEX:
			CLEAR_MUTEX_FLAG_ISEXCLUDED(handle);
			break;
		case TRACE_CLASS_TASK:
			CLEAR_TASK_FLAG_ISEXCLUDED(handle);
			break;
		}
    }
